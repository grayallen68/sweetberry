const dessertsList = [
  ...
  //go through desserts and give each element a dessert tag
  for(let i=0; i<dessertsList.length; i++){
    dessertsList[i].tag = "dessert"
  }
  //go through beverages and give each element a beverage tag
  for(let i=0; i<beveragesList.length; i++){
    beveragesList[i].tag = "beverage"
  }

  //create new data list by concatenating the beverage and dessert list
  const concatList = [...dessertsList ,...beveragesList];
  //go through the items and assign new ids
  for(let i=0; i<concatList.length; i++){
    concatList[i].id = "item" + i;
  }
